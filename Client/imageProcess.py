...
thought as sub-space to store names/variables that can be changed by any scope that can access it. Access being that the
reference is passed to that scope. However, be careful when programing with multi-threading because if there are threads
writing to the parameters within while another thread is accessing them can cause errors. Design the functions so there
...
any programs using this library are updated.
"""
###############################################################################
...
class ImageProcess:
    def __init__(self, parameters={'Parameters': 'values'}):
        """ ImageProcess:
    ...
    def __imageProcess0(self, data):
        ...
        ## Parameter Retrieval ##
        with self.paramsLock:
            param = self.parameters['Patameters']   # An example replace with something more apt.
